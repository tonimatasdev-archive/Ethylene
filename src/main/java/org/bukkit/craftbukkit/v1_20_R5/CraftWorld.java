...
import org.bukkit.craftbukkit.v1_20_R5.generator.structure.CraftGeneratedStructure;
import org.bukkit.craftbukkit.v1_20_R5.generator.structure.CraftStructure;
...
import org.bukkit.craftbukkit.v1_20_R5.metadata.BlockMetadataStore;
...
public class CraftWorld extends CraftRegionAccessor implements World {
    ...
    public boolean regenerateChunk(int x, int z) {
        ...
        ChunkHolder playerChunk = world.getPlayerChunkMap().getChunk(x, z);
        if (playerChunk != null) {
    ...
    public boolean createExplosion(double x, double y, double z, float power, boolean setFire, boolean breakBlocks, Entity source) {
        ...
        if (!breakBlocks) {
            explosionType = Level.ExplosionInteraction.NONE; // Don't break blocks
        } else if (source == null) {
            explosionType = Level.ExplosionInteraction.STANDARD; // Break blocks, don't decay drops
        } else {
            explosionType = Level.ExplosionInteraction.MOB; // Respect mobGriefing gamerule
        }
    ...
    public void playSound(Location loc, String sound, org.bukkit.SoundCategory category, float volume, float pitch, long seed) {
        ...
        ClientboundSoundPacket packet = new ClientboundSoundPacket(Holder.direct(SoundEvent.createVariableRangeEvent(new ResourceLocation(sound))), SoundSource.valueOf(category.name()), x, y, z, volume, pitch, seed);
